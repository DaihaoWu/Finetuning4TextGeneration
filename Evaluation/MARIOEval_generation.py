...
def load_lora_stablediffusion(model_path):
    from diffusers import StableDiffusionPipeline
    pipe = StableDiffusionPipeline.from_pretrained("CompVis/stable-diffusion-v1-4", torch_dtype=torch.float16)
    pipe.unet.load_attn_procs(model_path)
    return pipe

def test_lora_stablediffusion(prompt, save_path, num_images_per_prompt=4, pipe=None, generator=None):
    pipe.to("cuda")
    images = pipe(prompt, num_inference_steps=30, generator=generator, num_images_per_prompt=num_images_per_prompt,guidance_scale=7.5).images
    for idx, image in enumerate(images):
        image.save(save_path.replace('.jpg', '_' + str(idx) + '.jpg').replace('/images/', '/images_'+ str(idx) +'/'))
...
def MARIOEval_generate_results(root, dataset, method='sd-texted-model-lora', num_images_per_prompt=4, split=0, total_split=1):
    ...
    if method == 'controlnet':
        ...
        stage_1, stage_2, stage_3 = load_deepfloyd_if() 
    elif method == 'lora_stablediffusion' or method == 'enhanced_lora_stablediffusion':
        model_path = "../diffuser_docs/sd-texted-model-lora" # modify this to desired lora model
        pipe = load_lora_stablediffusion(model_path)
    ...
    for idx, prompt in tqdm(enumerate(prompts)): 
        ...
        if  method == 'controlnet':
            ...
            test_lora_stablediffusion(prompt=prompt, num_images_per_prompt=num_images_per_prompt,
                                  save_path=os.path.join(root_res, dataset, 'images', str(idx) + '.jpg'),
                                  pipe=pipe, generator=generator)
...
def parse_args():
    ...
    parser.add_argument(
        "--method",
        type=str,
        default='lora_stablediffusion', 
        required=False,
        choices=['controlnet', 'deepfloyd', 'stablediffusion', 'textdiffuser', 'lora_stablediffusion', 'enhanced_lora_stablediffusion']
    )
    parser.add_argument(
        "--gpu",
        type=int,
